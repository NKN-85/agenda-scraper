...
def convertir_fecha_capitol(texto):
    meses = {
        "enero": 1,
        "febrero": 2,
        "marzo": 3,
    ...
    patrones = [
        ...
        r"días?\s+(\d{1,2})\s+y\s+\d{1,2}\s+de\s+([a-záéíóú]+)\s+de\s+(\d{4})",
    ...
    m = re.search(
        r"y\s+del\s+\d{1,2}\s+de\s+[a-záéíóú]+\s+al\s+(\d{1,2})\s+de\s+([a-záéíóú]+)\s+de\s+(\d{4})",
        texto
    )
    if m:
        dia = int(m.group(1))
    ...
    m = re.search(
        r"(\d{1,2})\s+y\s+\d{1,2}\s+de\s+([a-záéíóú]+)\s*,\s*\d{1,2}\s+y\s+\d{1,2}\s+de\s+[a-záéíóú]+\s+de\s+(\d{4})",
        texto
    )
    if m:
        dia = int(m.group(1))
    ...
    m = re.search(
        r"(\d{1,2})\s+de\s+([a-záéíóú]+)\s*,\s*\d{1,2}\s+y\s+\d{1,2}\s+de\s+[a-záéíóú]+\s+de\s+(\d{4})",
        texto
    )
    if m:
        dia = int(m.group(1))
...
def sacar_fecha_desde_pagina_evento(session, url_evento):
    ...
    for linea in lineas[:150]:
        fecha = convertir_fecha_capitol(linea)
        if fecha:
...
def sacar_capitol():
    url = "https://gruposmedia.com/capitol-gran-via/"
    eventos = []
    vistos = set()
    ...
    for titulo, url_evento, idx in candidatos_unicos:
        ...
        for j in range(idx + 1, min(idx + 15, len(lineas))):
            candidato = lineas[j]
            fecha_evento = convertir_fecha_capitol(candidato)
            if fecha_evento:
                break
