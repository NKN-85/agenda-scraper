...
def convertir_fecha_maravillas(texto):
    meses = {
        "enero": 1,
        "febrero": 2,
        "marzo": 3,
    ...
    m = re.search(r"hasta\s+el\s+(\d{1,2})\s+de\s+([a-záéíóú]+)\s+de\s+(\d{4})", texto)
    if m:
        dia = int(m.group(1))
        mes = meses.get(m.group(2))
        anio = int(m.group(3))
    ...
    m = re.search(r"días?\s+(\d{1,2})\s+y\s+\d{1,2}\s+de\s+([a-záéíóú]+)\s+de\s+(\d{4})", texto)
    if m:
        dia = int(m.group(1))
        mes = meses.get(m.group(2))
        anio = int(m.group(3))
...
def sacar_fecha_desde_pagina_evento(session, url_evento):
    ...
    for linea in lineas[:150]:
        fecha = convertir_fecha_maravillas(linea)
        if fecha:
...
def sacar_maravillas():
    url = "https://gruposmedia.com/teatro-maravillas/"
    eventos = []
    vistos = set()
    ...
    for titulo, url_evento, idx in candidatos_unicos:
        ...
        for j in range(idx + 1, min(idx + 12, len(lineas))):
            candidato = lineas[j]
            fecha_evento = convertir_fecha_maravillas(candidato)
            if fecha_evento:
                break
